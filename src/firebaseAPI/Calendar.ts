...
export default class Calendar implements ICalendar<ITask> {
    ...
    private readonly firebaseConfig = {
        ...
        databaseURL: "https://akkuratovcalendar-default-rtdb.europe-west1.firebasedatabase.app"
    };
    ...
    private async init() {
        get(this.database).then((snapshot) => {
            if (snapshot.exists()) {
                ...
                throw new CalendarError(`There is no reference with namespace "${this.namespace}" in database.`);
            }
        }).catch((error) => {
            throw new CalendarError(`Couldn't read data from Firebase database with error: ${error}`);
        });
    }
    ...
    async filterTasks(filter: IFilter<ITask>, ...param: never[]): Promise<ITask[]> {
        return new Promise<ITask[]>((resolve, reject) => {
